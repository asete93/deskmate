...
**터미널 메뉴**는 서버 셸에 웹에서 직접 접속하는 화면입니다(워크스페이스 디렉터리에서 시작). 파일 확인·git 조작·로그 열람 등을 에이전트 없이 직접 할 때 사용합니다. **기본 off** — 설정 → 메뉴 표시의 "터미널" 토글로 켜야 메뉴·기능·연결이 모두 열립니다. 같은 곳의 **Git 메뉴·파일** 토글도 첫 배포·전체 초기화 시 기본 off이며 필요할 때 켭니다(파일을 끄면 파일 API도 차단).
...
- **서비스 연결** — 다른 포트/서버의 Deskmate 인스턴스를 등록하고 사이드패널 상단에서 전환(이름 변경 가능). 프로젝트마다 `--name`으로 띄운 인스턴스들을 한 브라우저에서 오갑니다.
- **언어 / Language** — 한국어/English. **UI 표시뿐 아니라 팀(에이전트)의 지침·보고 언어까지 전환**됩니다(영어 지침서가 별도로 내장). 전환 후에는 새 방 또는 대화 초기화 상태에서 가장 확실하게 적용됩니다.
- **전체 기억 초기화** (위험 구역) — 팀장(모든 방)·팀원 전원의 세션 기억만 리셋합니다. 대화 기록·티켓·결재·워크스페이스 파일은 그대로 남고, 대기 중이던 질문 카드는 취소됩니다. 에이전트가 맥락이 꼬였거나 세션이 비대해져 토큰이 낭비될 때 사용하세요.
