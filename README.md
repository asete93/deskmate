...
터미널에서 Claude Code를 1:1로 쓰는 대신, **여러 에이전트를 조직으로 묶어 운영**하고 싶을 때 사용합니다.

- 개발·문서·디자인 작업을 **역할별 AI 팀원**에게 나눠 맡기고, 팀장이 브리프 작성과 결과 검증을 담당
- 진행 상황을 **티켓 보드·요청(REQ)·보고서**로 추적하고, 중요한 결정은 **결재**로 승인
- 웹 산출물은 화면 위에 **핀을 꽂아 리뷰**하고, 수정 지시가 구조화된 문서로 팀에 전달
- 브라우저만 있으면 어디서든(모바일 포함) 서버의 AI 팀에 지시하고 파일·터미널·Git까지 관리

한 문장으로: **"Claude Code 위에 회사를 하나 세우는 도구"** 입니다.

## ⚠ 보안 — 반드시 읽어주세요

**이 대시보드에 접근할 수 있는 사람은 곧 서버에서 명령을 실행할 수 있는 사람입니다.**
에이전트·웹 터미널·파일 편집이 모두 서버 셸 권한으로 동작하므로, 접근 제어 없이 외부에 노출하면 서버 전체가 넘어갑니다. 아래 장치를 반드시 조합해 쓰세요.

| 장치 | 사용법 | 설명 |
|---|---|---|
| **로그인** | 설정 → 로그인 on | 단일 비밀번호(scrypt 해시 저장). 실패 5회 = 15분 잠금 + 전역 시도 제한. API·파일·WebSocket·터미널 전부 보호. **공개망 배포 시 필수** |
| **비밀번호 분실 복구** | `touch ~/.claude-control/<name>/reset-password` | 서버 셸 접근 = 소유자 증명. 다음 로그인 시도 때 초기화됩니다 |
...
정직하게: **오케스트레이션 자체는 토큰을 더 씁니다.** 대신 아낄 수 있는 장치를 함께 제공합니다.

**더 쓰는 부분**
- 팀장의 판단 턴 — 요청 분류, 브리프 작성, 팀원 결과 검증이 각각 LLM 턴입니다 (1:1 대화엔 없는 오버헤드)
- 방·팀원마다 독립 세션 — 각자의 컨텍스트를 유지하므로 세션 수만큼 컨텍스트 비용이 있습니다
- 커밋 메시지 자동 생성(선택) — Haiku 1회 호출 (미미함, 비워두고 커밋할 때만)

**절약하는 장치**
- **방별 모델·effort 오버라이드** — 잡담·간단한 방은 Haiku/low로, 중요한 방만 고성능으로
- **팀원별 모델 차등** — 단순 반복 작업 팀원은 Haiku, 설계 팀원만 Opus 계열로 고용 (고용 결재 화면에서 조정)
- **전체 기억 초기화 / 방별 대화 초기화** — 세션이 비대해져 매 턴 컨텍스트가 커질 때 리셋 (데이터는 유지 옵션 선택 가능)
...
- **대표(당신)**: 목표 설정, 지시, 결재, 산출물 검토. 유일한 사람.
- **팀장(메인 에이전트)**: 요청 분석 → 작업 분해 → 팀원 위임 → 결과 검증 → 보고. **파일 편집 툴이 물리적으로 차단**되어 있어 직접 구현할 수 없습니다 — 반드시 위임합니다.
- **팀원(서브 에이전트)**: 브리프를 받아 구현·검증하고 팀장에게 보고. 각자 독립 세션·역할·커스텀 지침을 가집니다.
...
이 외의 경로(에이전트가 몰래 서브에이전트를 만드는 것 등)는 서버가 차단합니다. 해고 역시 결재를 거칩니다.

### 채팅방 — 방마다 독립된 두뇌

- 방을 만들면 팀장의 **세션(기억)이 방 단위로 분리**됩니다. 주제가 섞이지 않고, 방을 지워도 다른 방은 무사합니다.
